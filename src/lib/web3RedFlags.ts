...
const RULES: Rule[] = [
  ...
  {
    id: 'user-inadequate-dev-learning-support',
    title: 'Weak support for learning (docs, onboarding, education)',
    detail:
      'The flag is thin tutorials, missing onboarding, or no investment in teaching devs — not that Solidity, Rust, Move, or other stacks are “bad languages”.',
    weight: 10,
    test: ({ folded: f }) => matchesAny(f, inadequateDevLearningSupport),
  },
  {
  ...
  {
    id: 'user-limited-on-chain-retention',
    title: 'Limited on-chain user retention',
    detail:
      'Users churn after incentives, one-off wallets, or weak sticky behavior — activity may not be durable.',
    weight: 10,
    test: ({ folded: f }) => matchesAny(f, limitedOnChainUserRetention),
  },
  {
    id: 'user-token-unlock-pressure-over-time',
    title: 'High token unlock pressure over time',
    detail:
      'Long vesting schedules or rolling unlocks can mean years of supply hitting the market — distinct from a single cliff.',
    weight: 11,
    test: ({ folded: f }) => matchesAny(f, tokenUnlockPressureOverTime),
  },
  {
    id: 'user-ecosystem-fragmentation',
    title: 'Ecosystem fragmentation (many projects, low cohesion)',
    detail:
      'Siloed teams, duplicate apps, or no shared roadmap — harder to compound network effects.',
    weight: 10,
    test: ({ folded: f }) => matchesAny(f, ecosystemFragmentation),
  },
  {
    id: 'user-weak-pmf-defi',
    title: 'Weak product–market fit in core DeFi',
    detail:
      'Core DEX/lending/perps usage looks hollow vs hype — PMF is a process, not a tweet.',
    weight: 10,
    test: ({ folded: f }) => matchesAny(f, weakProductMarketFitDefi),
  },
  {
    id: 'user-slow-adaptation-market-cycles',
    title: 'Slow adaptation to market cycles',
    detail:
      'Late pivots or mis-timed strategy vs macro/liquidity regimes — teams can miss the window.',
    weight: 10,
    test: ({ folded: f }) => matchesAny(f, slowAdaptationMarketCycles),
  },
  {
    id: 'user-partnership-hype-no-outcomes',
    title: 'Overhyped partnerships without tangible outcomes',
    detail:
      'MOUs and logo slides without shipped integrations — verify what actually changed on-chain or in product.',
    weight: 10,
    test: ({ folded: f }) => matchesAny(f, partnershipHypeNoOutcomes),
  },
  {
